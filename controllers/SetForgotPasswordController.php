<?php
class SetForgotPasswordController extends SetController{
	...
	*/
	function processForgot($f3){
		...
		if ($f3->exists('POST.isPost')){
			...
			if ($this->users->email != ''){
				...
				$this->emailResetLink($email, $this->users->username, $forgotPassId);
	...
	private function emailResetLink($toEmail, $toN, $forgotPass)
	{
		$qMAPI = 'http://quantumfi.net/api/sendmail/process_sendgrid.php';
		$url = "http://$_SERVER[HTTP_HOST]/resetPassword/$forgotPass";
		$to = $toEmail;
		$frN = 'Quantumfi Bot';
		$fr = 'no-reply@quantumfi.net';
		$sbj = "Re: SET Dev forgot password ($forgotPass)";
		$msg = 'Dear '.$toN.', <br/><br/>Please click the link below to reset your password: </br><a href="'.$url.'">'.$url.'</a><br/><br/>This is an auto-generated email. Please don\'t reply to this email.<br/><br/>Sincerely,<br/>SET Dev team<br/><br/>';
		
		$cnt_array = array('fr'=>$fr, 'to'=>$to, 'subject'=>$sbj, 'message'=>$msg, 'frName'=>$frN, 'toName'=>$toN);
